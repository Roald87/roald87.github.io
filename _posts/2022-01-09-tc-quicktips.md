...
- You can use [bit access](https://infosys.beckhoff.com/content/1033/tc3_plc_intro/2529343371.html?id=6082056588980779846) for integer variables. For example, `variable.2 := TRUE`, sets the third bit to `TRUE`.  There is read access as well. You can for example check if some bit of a number is true by `IF variable.3 THEN ...`. This method allows you to decipher an error code where each bit represents a specific error. Note that [bit access can be significantly slower](https://infosys.beckhoff.com/content/1033/tc3_plc_intro/2529442699.html?id=5627824830837717856) than using for example a bit mask.
